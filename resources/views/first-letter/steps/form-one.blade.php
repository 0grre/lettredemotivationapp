<section>
    ...
    <form method="post" action="{{ route('letters.create.step.one.post') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-error field="job" class="mt-2 text-sm text-red-600 dark:text-red-500"/>
        </div>
        <div>
            <select required name="experience" id="experience"
                    class="mt-1 block w-full focus:outline-none border py-3 appearance-none bg-slate-50 border-slate-200 focus:bg-white focus:border-accent-500 focus:ring-accent-500 placeholder-slate-400 px-3 rounded-xl sm:text-sm text-accent-500">
